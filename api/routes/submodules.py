...
def upsert_production_chart(
    ...
    current_user: dict = Depends(require_permissions(Permission.ORDER_LIFECYCLE_WRITE))
):
...
def get_production_chart(
    order_id: UUID,
    client: Client = Depends(get_supabase),
    current_user: dict = Depends(require_permissions(Permission.ORDER_LIFECYCLE_READ)),
):
...
def create_ancillary_equipment(
    order_id: UUID,
    item_in: schemas.AncillaryEquipmentBase,
    client: Client = Depends(get_supabase),
    current_user: dict = Depends(require_permissions(Permission.ORDER_LIFECYCLE_WRITE)),
):
...
def create_site_verification(
    order_id: UUID,
    item_in: schemas.SiteVerificationBase,
    client: Client = Depends(get_supabase),
    current_user: dict = Depends(require_permissions(Permission.ORDER_LIFECYCLE_WRITE)),
):
...
def create_packing_list(
    order_id: UUID,
    item_in: schemas.PackingListBase,
    client: Client = Depends(get_supabase),
    current_user: dict = Depends(require_permissions(Permission.ORDER_LIFECYCLE_WRITE)),
):
...
def create_installation_record(
    order_id: UUID,
    item_in: schemas.InstallationRecordBase,
    client: Client = Depends(get_supabase),
    current_user: dict = Depends(require_permissions(Permission.ORDER_LIFECYCLE_WRITE)),
):
...
@router.patch("/{order_id}/installation-record", response_model=schemas.InstallationRecordResponse)
def update_installation_record(
    order_id: UUID,
    item_in: schemas.InstallationRecordUpdate,
    client: Client = Depends(get_supabase),
    current_user: dict = Depends(require_permissions(Permission.ORDER_LIFECYCLE_WRITE)),
):
    return SubmoduleService(client, SUBMODULE_TABLES["InstallationRecord"]).update(order_id, item_in)

@router.get("/{order_id}/installation-record", response_model=schemas.InstallationRecordResponse)
def get_installation_record(
    order_id: UUID,
    client: Client = Depends(get_supabase),
    current_user: dict = Depends(require_permissions(Permission.ORDER_LIFECYCLE_READ)),
):
